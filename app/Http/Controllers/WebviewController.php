...
class WebviewController extends Controller
{
    ...
    public function mainview()
    {
        ...
        $ad_two = Addbanner::where('status', 'Active')->whereIn('id', ['2'])->first();
        $ad_three = Addbanner::where('status', 'Active')->whereIn('id', ['3'])->first();
        $ad_four = Addbanner::where('status', 'Active')->whereIn('id', ['4'])->first();
        $ad_five = Addbanner::where('status', 'Active')->whereIn('id', ['5'])->first();
   ...
   public function shopPage(Request $request)
    ...
    public function getsubcategoryproduct(Request $request)
    {
        ...
        if (isset($request->price_range)) {
            ...
            $subcategoryproducts = Product::where('subcategory_id', $subcategory->id)->where('status', 'Active')->whereBetween('ProductSalePrice', $num)->orderby('position', 'desc')->get();
        } else {
            $subcategoryproducts = Product::where('subcategory_id', $subcategory->id)->where('status', 'Active')->orderby('position', 'desc')->get();
        }
    ...
    public function quick($id)
    {
        ...
        $sizesolds = Size::where('product_id', $productdetails->id)->where('status', 'Active')->get();
        $weightolds = Weight::where('product_id', $productdetails->id)->get();
        return view('webview.content.product.quick', ['sizesolds' => $sizesolds, 'weightolds' => $weightolds, 'singlemain' => $singlemain, 'varients' => $varients, 'relatedproducts' => $relatedproducts, 'productdetails' => $productdetails]);
    }

    public function newslatter_post(Request $request)
    {
        $request->validate([
            'email' => 'required',
        ]);
